def get_recipe(path, search_id):
    with open(path, "r") as file:
        ...
        for receipt in receipts:
            receipt_dict = {
                "id": receipt.split(",", 2)[0],
                "name": receipt.split(",", 2)[1],
                "ingredients": receipt.strip().split(",", 2)[-1].split(",")
            }
            list_of_receipts.append(receipt_dict)

        for i in range(len(list_of_receipts)):
            if list_of_receipts[i]["id"] == search_id:
                return list_of_receipts[i]
