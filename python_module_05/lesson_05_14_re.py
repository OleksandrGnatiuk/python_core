...
def find_all_phones(text):
    pattern = r"\+380\(\d\d\)[\d]{3}-[\d]{1}-[\d]{3}|\+380\(\d\d\)[\d]{3}-[\d]{2}-[\d]{2}"
    result = re.findall(pattern, text)
    return result


if __name__ == "__main__":
    text = """шаблон якого наступний: +380(67)777-7-777 або +380(67)777-77-77. Напишіть регулярний вираз для функції"""
...
#
# Напишіть регулярний вираз для функції find_all_phones, яка буде в тексті (параметр text) знаходити всі
# телефонні номери за вказаним шаблоном та повертати список отриманих з тексту збігів.
#
