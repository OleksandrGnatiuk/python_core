...
def find_articles(key, letter_case=False):
    ...
    for dct in articles_dict:
        if letter_case:
            if key in dct["title"] or key in dct["author"]:
                new_list.append(dct)
        else:
            if key.lower() in dct["title"].lower() or key.lower() in dct["author"].lower():
                new_list.append(dct)
    return new_list



# Ваша компанія веде блог.
# Треба реалізувати функцію find_articles для пошуку за статтями цього блогу.
