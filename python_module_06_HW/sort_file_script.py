...
CYRILLIC_SYMBOLS = "абвгдеёжзийклмнопрстуфхцчшщъыьэюяєіїґ?<>,!@#[]#$%^&*()-=; " # символи отнаковы на всых мовах)
...
def sort_file(path):
    ...
    for _ in range(2): # Краще не два цикли, а окрему функцію для видалення файлів - так більш зрозуміло
        # потрібно два цикла, для видалення пустих папок, що залишаються після рекурсивного обходу, а також для розпаковки архівів.
        for i in p.iterdir(): # подивіться в сторону методу glob
            # ці імена краще взяти з ключів словника. if name in suffix_dict
...
def main(path):
    ...
    # Ці дані поминні певернутись як результат роботи Ваших функцій
...
if __name__ == "__main__":
    # Тут потрібно зробити перевірку на введення цього параметру, бо його може і не  буть, і тоді вся програма - впаде
